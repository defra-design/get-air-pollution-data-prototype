...
d3.csv("/public/javascripts/chart-data/jdi_data_daily_copy.csv").then(function (data) {
...
/*   tooltip.html(`<strong style="font-size:19px;">${d.population !== undefined ? (d.population / 1000).toFixed(0) : 'N/A'} μg/m3</strong><div style="display: block; margin-top: 2px;">${formatDate(d.date)}</div>`); */

tooltip.html(`<strong style="font-size:19px;">${d.population === 0 ? 'No data' : (d.population / 1000).toFixed(0) + ' μg/m3'}</strong><div style="display: block; margin-top: 2px;">${formatDate(d.date)}</div>`);
