...
// Set dimensions and margins for the pm10
  function drawChart() {
    ...
    const width = document.getElementById("pm10-container-month").clientWidth - margin.left - margin.right; // Use container's width
...
// Create the SVG element and append it to the pm10 container
const svg = d3.select("#pm10-container-month")
  .append("svg")
...
//Load data
  d3.csv("/public/javascripts/version_7/chart-data/past-month/pm10.csv").then(function (data) {
...
const area = d3.area()
  ...
  .x(d => x(d.date))
  .y1(d => y(d.population))
  .y0(height); // The lower boundary goes down to the x-axis (bottom of the pm10)
...
// Create event listeners for the pm10-container-month to show/hide elements
d3.select("#pm10-container-month")
  .on("mouseover", function () {
    // Show the elements when the mouse enters the pm10 container
  ...
  .on("mouseout", function () {
    // Hide the elements when the mouse leaves the pm10 container
