...
// Set dimensions and margins for the ozone
  function drawChart() {
    // Clear previous ozone elements if any
    d3.select("#ozone-container-month").selectAll("*").remove();
  
    // Set dimensions and margins for the ozone
    const margin = { top: 20, right: 10, bottom: 50, left: 40 };
    
    // Use the full width of the container
    const width = document.getElementById("ozone-container-month").clientWidth - margin.left - margin.right; // Use container's width
...
// Create the SVG element and append it to the ozone container
const svg = d3.select("#ozone-container-month")
  .append("svg")
...
//Load data
  d3.csv("/public/javascripts/version_7/chart-data/past-month/ozone.csv").then(function (data) {
...
const area = d3.area()
  ...
  .x(d => x(d.date))
  .y1(d => y(d.population))
  .y0(height); // The lower boundary goes down to the x-axis (bottom of the ozone)
...
// Create event listeners for the ozone-container-month to show/hide elements
d3.select("#ozone-container-month")
  .on("mouseover", function () {
    // Show the elements when the mouse enters the ozone container
  ...
  .on("mouseout", function () {
    // Hide the elements when the mouse leaves the ozone container
