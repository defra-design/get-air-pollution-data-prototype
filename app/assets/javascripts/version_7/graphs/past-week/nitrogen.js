...
// Set dimensions and margins for the chart
  function drawChart() {
    ...
    d3.select("#chart-container-week").selectAll("*").remove();
  
    // Set dimensions and margins for the chart
    const margin = { top: 20, right: 10, bottom: 50, left: 40 };
    
    // Use the full width of the container
    const width = document.getElementById("chart-container-week").clientWidth - margin.left - margin.right; // Use container's width
...
const svg = d3.select("#chart-container-week")
  .append("svg")
...
//Load data
  d3.csv("/public/javascripts/version_7/chart-data/past-week/no2.csv").then(function (data) {
...
// Create event listeners for the chart-container-week to show/hide elements
d3.select("#chart-container-week")
...
tooltip.html(`<strong style="font-size:22px;">${d.population === 0 ? 'No data' : (d.population / 1000).toFixed(0) + ' μg/m3'}</strong><div style="display: block; margin-top: 2px; font-size: 19px;">${formatDateTime(d.date)}</div>`);
