...
// Set dimensions and margins for the chart
  function drawChart() {
    ...
    d3.select("#ozone-container-2024").selectAll("*").remove();
  
    // Set dimensions and margins for the chart
    const margin = { top: 20, right: 10, bottom: 40, left: 40 };
    
    // Use the full width of the container
    const width = document.getElementById("ozone-container-2024").clientWidth - margin.left - margin.right; // Use container's width
...
const svg = d3.select("#ozone-container-2024")
  .append("svg")
...
d3.csv("/public/javascripts/version_7/chart-data/2024/ozone.csv").then(function (data) {
...
labelGroup.append("foreignObject")
  ...
  .attr("y", y(120) - 65) // Position it slightly above the line
  ...
  .html("Exceedance over 120 µg/m3"); // Set the text inside the div
...
// Create event listeners for the ozone-container-2024 to show/hide elements
d3.select("#ozone-container-2024")
...
tooltip.html(`<strong style="font-size:22px;">${d.population === 0 ? 'No data' : (d.population).toFixed(0) + ' μg/m3'}</strong><div style="display: block; margin-top: 2px; font-size: 19px;">${formatDate(d.date)}</div>`);
