...
// Set dimensions and margins for the pm2_5
  function drawChart() {
    // Clear previous pm2_5 elements if any
    d3.select("#pm2_5-container-month").selectAll("*").remove();
  
    // Set dimensions and margins for the pm2_5
    const margin = { top: 20, right: 10, bottom: 50, left: 40 };
    
    // Use the full width of the container
    const width = document.getElementById("pm2_5-container-month").clientWidth - margin.left - margin.right; // Use container's width
...
// Create the SVG element and append it to the pm2_5 container
const svg = d3.select("#pm2_5-container-month")
  .append("svg")
...
//Load data
  d3.csv("/public/javascripts/version_7/chart-data/past-month/pm2_5.csv").then(function (data) {
...
svg.append("g")
   ...
   .call(d3.axisBottom(x)
     .ticks(d3.timeDay.every(8)) // Display ticks every 6 hours
     .tickFormat(d => {
...
svg.selectAll(".tick text")
  ...
  // Add the y-axis
    ...
    .style("font-size", "16px")
    .call(d3.axisLeft(y)
    .ticks(3) // Adjust the number of ticks as needed
...
labelGroup.append("foreignObject")
  ...
  .attr("y", y(200) - 65) // Position it slightly above the line
...
const area = d3.area()
  ...
  .x(d => x(d.date))
  .y1(d => y(d.population))
  .y0(height); // The lower boundary goes down to the x-axis (bottom of the pm2_5)
...
// Create event listeners for the pm2_5-container-month to show/hide elements
d3.select("#pm2_5-container-month")
  .on("mouseover", function () {
    // Show the elements when the mouse enters the pm2_5 container
  ...
  .on("mouseout", function () {
    // Hide the elements when the mouse leaves the pm2_5 container
